...
如果两个操作访问同一个变量，并且两个操作中有一个为写操作，那么这两个操作之间存在数据依赖性，这种情况下只要重排序程序的执行结果就会改变。
...
<font color="red">as-if-serial语义：不管如何重排序（编译器和处理器为了提高并行度），（单线程）程序的执行结果不能被改变，处理器和编译器都不许遵循as-if-serial语义（不会对存在数据依赖关系的操作做重排序）。</font>
