...
<font color="red">乐观锁认为对同一数据的并发操作并不会总是发生，属于小概率时间，并不需要总是对数据上锁</font>，但是在更新数据的时候会判断在此期间别的线程有没有修改数据。也就是不采用数据库本身的锁机制，而是通过代码来实现线程安全。在程序上我们可以采用<font color="red">版本号机制</font>或者<font color="red">CAS机制</font>来实现。
