...
5. 局部唯一性
   ...
   最重要的一点，自增ID是局部唯一，只在当

   存在自增ID回溯的问题

2. 安全性不高
前数据库实例中唯一，而不是全局唯一，在任意服务器间都 是唯一的。对于目前分布式系统来说，这简直就是噩梦。
...
写写情况，即并发事务同时对相同数据做出改动。这种情况下会发生<font color="red">脏写</font>，任何一种隔离级别都不允许脏写问题的发生。所以在多个未提交的事务同时对同一条记录做出修改时，需要让它们<font color="red">排队执行</font>，这里的排队的过程本质上是通过锁机制来实现的。（这里的锁其实是内存的一个数据结构）。
...
4. 元数据锁
   ...
   元数据锁即Meta Data Lock，简称MDL锁，[属于表锁的范畴]()。当对一个表做增删改查操作的时候，加 MDL读锁；当要对表做结构变更操作的时候，加 MDL写锁。

##### 页级锁

页锁是在页的粒度上进行加锁，锁定的数据资源比行锁要更多，因为一个页上有多个行记录。<font color="red">页锁的开销介于行锁和表锁之间，会出现死锁的情况，并发度一般。</font>

因为每个层级的锁空间大小是有限的，锁会占用内存空间，当某个层级的锁占用的内存空间超过阈值时会出现<font color="red">锁升级</font>的现象。（一般锁升级过程是用更大粒度的锁代替更小粒度的锁）<font color="red">锁升级可以降低小粒度锁的内存空间但是数据的并发度也下降了。</font>

##### 行锁

#### 3.3 从对待锁的态度划分

从对待锁的维度划分可以分为：悲观锁、乐观锁。需要注意的是悲观锁、乐观锁并不是锁而是<font color="red">设计思想</font>。

##### 悲观锁（Pessimistic Locking）

<font color="red">悲观锁总是假设最坏的情况，每次拿数据的时候都会认为别的线程会修改数据，所以在每次拿数据之前都会加锁</font>，这样别的事务就会阻塞直到它获取到锁为止（共享资源每次只会被一个线程访问，其他线程处于阻塞状态，用完之后再释放锁）。比如行锁、表锁、读锁、写锁在操作之前都会加锁。Java中的`synchronized`和`ReentrantLock`等独占锁就是悲观锁思想的实现。

##### 乐观锁（Optimistic Locking）

<font color="red">乐观锁认为对同一数据的并发操作并不会总是发生，属于小概率事件，并不需要总是对数据上锁</font>，但是在更新数据的时候会判断在此期间别的线程有没有修改数据。也就是不采用数据库本身的锁机制，而是通过代码来实现线程安全。在程序上我们可以采用<font color="red">版本号机制</font>或者<font color="red">CAS机制</font>来实现。

<font color="red">乐观锁适用于读操作多的应用类型，这样可以提高系统的吞吐量。</font>

在Java中atomic包中的原子变量类就是就CAS实现的乐观锁。

**数据库中的乐观锁的实现方式**

1. 版本号机制

   在表中设计一个<font color="red">version字段</font>，第一次读的时候会获取version字段的值。然后对数据更新或者删除的时候会执行 `UPDATE ... SET version = version + 1 WHERE version = version`，如果此时有其他事务对此条数据做了修改，则当前事务不会执行成功。

2. 时间戳机制

   时间戳机制和版本号机制一样，也是在更新提交的时候，将当前数据的时间戳和更新之前的时间戳做比较，如果两者一致则更新成功，否则说明发生版本冲突，更新失败。

以上两种方式是开发者自己控制数据并发操作的权限，都是通过给数据添加一个字段（版本号/时间戳）从而确认当前拿到的数据是否被更新。
...
全局锁就是对整个<font color="red">数据库实例加锁</font>，是数据库处于<font color="red">只读状态</font>。此时数据库的更新语句（增删改）、数据定义语句（建表、修改表结果） 和更新事务类的提交语句都处于阻塞状态

引用场景：<font color="red">全库逻辑备份</font>
...
死锁是指两个或者多个事务在同一资源上互相占用，并且请求锁定对方占用的资源，从而导致恶性循环
...
MVCC （Multiversion Concurrency Control），多版本并发控制。顾名思义，MVCC可以通过数据行的多个版本管理来实现数据库的**并发控制**（保证数据的一致性）。

<font color="red">解决的问题：为了查询一些正在被另一个事务更新的行，并且可以查询到更新之前的值，这样在做查询的时候就不用等待另一个事务释放锁</font>

### 2.快照读与当前读

MVCC在MySQL InnoDB中的实现主要是为了<font color="red">提高数据库的并发性能</font>，用更好的方式去处理读写冲突，做到即使有读写冲突时也不用加锁，即<font color="red">非阻塞读</font>。而这个读指的就是`快照读` , 而非`当前读`，当前读实际上是一种加锁的操作，是悲观锁的实现。而MVCC本质是采用<font color="red">乐观锁</font>思想的一种方式。

#### 2.1快照读

快照读也叫做一致性读，读取的是快照数据，快照读是基于MVCC实现的。<font color="red">不加锁的简单的select都属于快照度，即不加锁的非阻塞读。</font>例如：

```mysql
select * from player where .....
```

之所以出现快照读的情况，是基于提高并发性能的考虑，快照读是基于MVCC实现的，它在很多情况下可以避免加锁情况，降低了读操作的开销。

既然是基于多版本实现的，那么快照读读取的数据不一定是最新的， 有可能是历史版本数据。

快照读的隔离级别不是串行级别，如果是串行级别那么快照度会退化到当前读。

#### 2.2当前读

当前读读取的是最新版本的数据。读取时还要保证其他并发事务不能修改当前记录，所以要对当前的记录进行加锁操作。加锁的select、增删改操作都会进行当前读（保证当前的记录是最新的）
...
<font color="red">MVCC实现原理依赖于：隐藏字段（事务ID）、 Undo Log、Read View</font>
