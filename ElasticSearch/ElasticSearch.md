...
分片是ElasticSearch最小的工作单元。传统的数据库每个字段只能存储单个数值，但是这并不能实现全文检索。文本中的每个单词需要被搜索，这就意味着数据库额每个字段的都要具有索引多值的能力，最好的实现方式就是建立`倒排索引`

##### 4.6.1 倒排索引

ElasticSearch使用的是倒排索引的数据结构，他适用于快速的全文检索

##### 4.6.2 文档检索

**倒排索引写入磁盘以后是不会发生变化的**，因为索引的不可变其具有以下价值：

1. 不需要锁。因为索引一旦建立好之后不可修改，所以它是线程安全的
